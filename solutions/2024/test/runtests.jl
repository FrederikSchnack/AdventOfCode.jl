using AdventOfCode.AdventOfCode24

@testset "Day 01" begin
    @test AdventOfCode24.Day01.day01() == [2970687, 23963899]
end

@testset "Day 02" begin
    @test AdventOfCode24.Day02.day02() == [356, 413]
end

@testset "Day 03" begin
    @test AdventOfCode24.Day03.day03() == [162813399, 53783319]
end

@testset "Day 04" begin
    @test AdventOfCode24.Day04.day04() == [2554, 1916]
end

@testset "Day 05" begin
    @test AdventOfCode24.Day05.day05() == [5651, 4743]
end
@testset "Day 06" begin
    @test AdventOfCode24.Day06.day06() == [5531, 2165]
end

@testset "Day 07" begin
    @test AdventOfCode24.Day07.day07() == [975671981569, 223472064194845]
end

@testset "Day 08" begin
    @test AdventOfCode24.Day08.day08() == [390, 1246]
end

@testset "Day 09" begin
    @test AdventOfCode24.Day09.day09() == [6378826667552, 6413328569890]
end

@testset "Day 10" begin
    @test AdventOfCode24.Day10.day10() == [593, 1192]
end

@testset "Day 11" begin
    @test AdventOfCode24.Day11.day11() == [203953, 242090118578155]
end

@testset "Day 12" begin
    @test AdventOfCode24.Day12.day12() == [1352976, 808796]
end

@testset "Day 13" begin
    @test AdventOfCode24.Day13.day13() == [32067, 92871736253789]
end

@testset "Day 14" begin
    @test AdventOfCode24.Day14.day14() == [230436441, 8270]
end

@testset "Day 15" begin
    @test AdventOfCode24.Day15.day15() == [1514353, 1533076]
end

@testset "Day 16" begin
    @test AdventOfCode24.Day16.day16() == [105508, 548]
end

@testset "Day 17" begin
    @test AdventOfCode24.Day17.day17() == ["7,3,1,3,6,3,6,0,2", 105843716614554]
end

@testset "Day 18" begin
    @test AdventOfCode24.Day18.day18() == [370, "65,6"]
end

@testset "Day 19" begin
    @test AdventOfCode24.Day19.day19() == [358, 600639829400603]
end

@testset "Day 20" begin
    @test AdventOfCode24.Day20.day20() == [1378, 975379]
end

@testset "Day 21" begin
    @test AdventOfCode24.Day21.day21() == [224326, 279638326609472]
end

@testset "Day 22" begin
    @test AdventOfCode24.Day22.day22() == [14869099597, 1717]
end

@testset "Day 23" begin
    @test AdventOfCode24.Day23.day23() == [1323, "er,fh,fi,ir,kk,lo,lp,qi,ti,vb,xf,ys,yu"]
end

@testset "Day 24" begin
    @test AdventOfCode24.Day24.day24() == [57344080719736, "cgq,fnr,kqk,nbc,svm,z15,z23,z39"]
end

@testset "Day 25" begin
    @test AdventOfCode24.Day25.day25() == [3249]
end
